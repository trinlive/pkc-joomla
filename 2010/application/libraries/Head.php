...
class Head {
    ...
    var $doctype = 'xhtml1-trans';    //Default Doctype
    ...
    var $meta_language = "th";       //Language for meta data
    ...
    var $use_favicon = FALSE;       //Should we use the favicon?
    ...
    function render_html() {
        if (in_array($this->doctype, $this->xml_doctypes)) {
            return '<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="' . $this->meta_language . '" lang="' . $this->meta_language . '">' . $this->bump();
        } else {
            return '<html>' . $this->bump();
        }
    }
...
<script type="text/javascript">
  ...
  (function() {
     ...
     * Header Class
     *
     * Asset management class witch provides and API to loading scripts, stylesheets and setting semantic and meta data
     *
     * @licence MIT Licence
     * @category Libraries
     * @author Dayton Nolan
     * @link http://www.daytonnolan.com
     * @version 2.0.0
     * test
     * -------------------------------------------------- */

    public function body_id($body_id = NULL) {
		if($body_id != ''){
        	$this->body_id = ' id="' . $body_id . '"';
		}
    }

    public function body_class($body_class = NULL) {
		if($body_class != ''){
        	$this->body_class = ' class="' . $body_class . '"';
		}
    }

    function render_body() {
        if ($this->body_id || $this->body_class) {
			$this->body_id($this->body_id);
			$this->body_class($this->body_class);
            return $this->body_id . $this->body_class;
        }
    }
