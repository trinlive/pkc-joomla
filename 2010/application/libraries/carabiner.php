...
class Carabiner {
	...
	public $combine_minify_js  = TRUE;
	public $minify_css = TRUE;
	public $combine_minify_css = TRUE;
	...
	*/		
	private function _display_js($group = 'main')
	{
		...
		// Use Minify (http://code.google.com/p/minify)
		if ($this->combine_minify_js) {
			if (isset($this->js[$group]) && is_array($this->js[$group]) && count($this->js[$group]) == 0) return;
			$_script = NULL;
					$_tag = NULL;
			foreach($this->js[$group] as $ref) {
						$ref['dev'] = str_replace(base_url(), '', $ref['dev']);
						$ref['dev'] = str_replace('assets/', '', $ref['dev']); // For js_view
						if ($this->isURL($ref['dev'])) {
							echo '<script type="text/javascript" src="'.$ref['dev'].'" charset="'.$this->CI->config->item('charset').'"></script>'."\r\n";
						} else {
							$dir = ( $this->isURL($ref['dev']) ) ? '' : $this->script_dir;
							if (!$ref['combine'] && !$ref['minify']) {
								$_script .= '<script type="text/javascript" src="'.base_url().$dir.$ref['dev'].'" charset="'.$this->CI->config->item('charset').'"></script>'."\r\n";
							} else {
								$_tag .= $dir . $ref['dev'] . ',';
							}
						}
					...
					if ($_tag != NULL) {
						$_tag = rtrim($_tag, ',');
						echo '<script type="text/javascript" src="'.base_url().'min/?f='.$_tag.'" charset="'.$this->CI->config->item('charset').'"></script>'."\r\n"; 
					}
		...
		// if we're combining files and minifying them
			...
			if ($num_combine > 0) echo $this->_tag('js', $filename, TRUE);
	...
	*/		
	private function _display_css($group = 'main')
	{
		...
		// Use Minify (http://code.google.com/p/minify)
		if ($this->combine_minify_css) {
			if (isset($this->css[$group]) && is_array($this->css[$group]) && count($this->css[$group]) == 0) return;
			$_tag = NULL;
			foreach($this->css[$group] as $media => $refs) {
				foreach($refs as $ref) {
					$ref['dev'] = str_replace(base_url(), '', $ref['dev']);
					$ref['dev'] = str_replace('assets/', '', $ref['dev']); // For css_view
					if ($this->isURL($ref['dev'])) {
						echo '<link type="text/css" rel="stylesheet" href="'.$dir.$_tag.'" media="'.$media.'" />'."\r\n";
					} else {
						$dir = ( $this->isURL($ref['dev']) ) ? '' : $this->style_dir;
						$_tag .= $dir . $ref['dev'] . ',';
					}
				}
			}
			$_tag = rtrim($_tag, ',');
			echo '<link type="text/css" rel="stylesheet" href="'.base_url().'min/?f='.$_tag.'" media="screen" />'."\r\n";
			return;
		}
		...
		if($this->dev){ // we're in a development environment
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach ($refs as $ref):
					...
					if(!$ref['combine']):
						echo (isset($ref['prod'])) ? $this->_tag('css', $ref['prod'], FALSE, $media) : $this->_tag('css', $ref['dev'], FALSE, $media);
					elseif(!$ref['minify']):
						$files[] = (isset($ref['prod'])) ? array('prod'=>$ref['prod'], 'dev'=>$ref['dev'], 'minify'=>$ref['minify'] ) : array('dev'=>$ref['dev'], 'minify'=>$ref['minify']);
					else:
						$files[] = (isset($ref['prod'])) ? array('prod'=>$ref['prod'], 'dev'=>$ref['dev'] ) : array('dev'=>$ref['dev']);
				...
				if ($num_combine > 0) echo $this->_tag('css',  $filename, TRUE, $media);
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach ($refs as $ref):
					...
					if($ref['combine'] == false):
						echo (isset($ref['prod'])) ? $this->_tag('css', $ref['prod'], $media) : $this->_tag('css', $ref['dev'], FALSE, $media);
					else:
						$files[] = (isset($ref['prod'])) ? array('prod'=>$ref['prod'], 'dev'=>$ref['dev'], 'minify'=>FALSE ) : array('dev'=>$ref['dev'], 'minify'=>FALSE);
					endif;
	...
	*/		
	public function empty_cache($flag = 'both', $before = 'now')
	{
		...
		$files = get_filenames($this->cache_path);
                $files_views = get_filenames($this->cache_path . 'views/');
				...
				foreach( $files as $file ){
					...
					if ( ($ext == 'js' || $ext == 'css') && $fl >= 42 && ( filemtime( $this->cache_path . $file ) < $before) ) {
						...
						if($success) : log_message('debug', 'Carabiner: Cache file '.$file.' was removed from '.$this->cache_path);
                                ...
                                foreach( $files_views as $file ){
					
					$ext = substr( strrchr( $file, '.' ), 1 );

					if ( ($ext == 'js' || $ext == 'css') && ( filemtime( $this->cache_path . 'views/' . $file ) < $before) ) {
						
						$success = unlink( $this->cache_path . 'views/' . $file );
						
						if($success) : log_message('debug', 'Carabiner: Cache file '.$file.' was removed from '.$this->cache_path . 'views/');
						else : log_message('error', 'Carabiner: There was an error removing cache file '.$file.' from '.$this->cache_path . 'views/');
						endif;
					}
