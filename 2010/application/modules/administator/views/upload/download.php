...
<table width="100%" border="0">
  ...
  <tr>
    ...
    ?>
    	<br><?php if(isset($filess['extension'])):?><img src="<?php echo site_assets_url('images/icons/'.$filess['extension'].'.gif');?>"><?php endif;?>
    	<a href="<?php echo site_url(admin_module('upload/'.$file_name.'?action=file&file='.$file.'&tg='.$_GET['tg']))?>" class="small1"><font color="green"><?php echo $file;?></font></a><br> 
    <?php 
    	}
    }
    closedir($dh);
  ...
  <tr>
	...
	<table width="100%">
		<tr>
			<td>
			<input type="button" name="button" id="button" value="เลือกไฟล์นี้" onclick="window.opener.document.getElementById('<?php echo $_GET['tg']?>').value='<?php echo $img_name;?>';window.close();">
			</td>
			<td>			
...
<form action="<?php echo site_url(admin_module('upload/'.$file_name.'?action=save&tg='.$_GET['tg']));?>" method="post" name="form1" target="_self" id="form1" enctype="multipart/form-data">
<table width="100%" border="0">
 <tr>
    <td width="100%">
