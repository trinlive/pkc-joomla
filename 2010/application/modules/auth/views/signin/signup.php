...
class Signup extends MY_Controller {
	...
	public function index($action=NULL) {
		...
		$view = array(
				'breadcrumb'=> array('include_segments' => array(site_url('call')=>'สายตรงเทศบาล','สมัครสมาชิกใหม่')),
	...
	private function _signup_action(){
		...
		$rules = array(
				array('field' => 'idcard', 'label' => 'กรุณากรอกรหัสบัตรประชาชน', 'rules' => 'trim|required'),
		);
	
		$this->form_validation->set_rules($rules);
		if ($this->form_validation->run() !== FALSE)
		{
			if ($this->recaptcha->check_answer($this->input->ip_address(),$this->input->post('recaptcha_challenge_field', TRUE),$this->input->post('recaptcha_response_field', TRUE))){
				...
				redirect('auth/signup/new?captcha=error');
			}
		}
	
		$view = array(
				'breadcrumb'=> array('include_segments' => array(site_url('call')=>'สายตรงเทศบาล','สมัครสามาชิกใหม่')),
				'exclude' => array($this->uri->segment(1),$this->uri->segment(2),$this->uri->segment(3)),
				'message' => $getdata['msg']
		);
