	...
	case 'update' :
		...
		ADOdb_Active_Record::SetDatabaseAdapter($db);
			...
			$news->date_news =$gettmp['date_news'];	
				...
				if ($handle->uploaded) :
					if ($news->file2 != "") :
								if(file_exists("../img_news/thumbnail/".$news->image1)) unlink("../img_news/thumbnail/".$news->image1);
								endif ;
					$handle->Process('../img_news/thumbnail/');
					if ($handle->processed) :
					$new_imagename = $handle->file_dst_name  ;
...
case 'delete' : 
				...
				$SQLstr = " SELECT * FROM `newses` WHERE `newses`.`nid` =  '".$gettmpnid."' ";
 				$rsdel = $db->Execute($SQLstr) ;
					...
					$pimage1 = $rsdel->fields['image1'] ;
						if ($pimage != "") :
							if(file_exists("../img_news/thumbnail/$pimage1")) unlink("../img_news/thumbnail/$pimage1");
					 	endif ;
		...
		if($news):
			...
			$getdata[msg].="<meta http-equiv=\"refresh\" content=\"2; URL=news_to_delete.php \">";	
