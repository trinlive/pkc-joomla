...
if($_GET[actions]=="del"):
	...
	$SQLstr = "SELECT * FROM products_images WHERE id = '".$gettmp[id]."' ";
	...
	if($products_image):
			echo "<meta http-equiv=\"refresh\" content=\"0; URL=image_product_to_delete3.php?tid=$tid\">";	
			saverecord('Delete products Image');	
	endif ;	
endif ;
?>
<?php
...
`products_images`.`id`,
`products_images`.`tid`,
`products_images`.`caption`,
`products_images`.`image1`,
`products_images`.`image2`,
`products_images`.`image_active`,
`products`.`name_product`
FROM
`products_images`
Inner Join `products` ON `products`.`tid` = `products_images`.`tid` WHERE `products_images`.`tid` = '".$tid."' ORDER BY products_images.id  ";
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="100%" height="29"  border="0" cellpadding="0" cellspacing="0" background="images/bg_head05.gif">
          <tr>
            <td width="228" height="29"><span class="arialWH18B" style="margin-left:8px;">Delete Image Product</span></td>
      ...
      <tr>
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr valign="top">
            <td width="8">&nbsp;</td>
            <td><form action="image_product_action.php" method="post" name="form1" id="form1" style="margin:0" onSubmit="return checkForm();">
			<?php if (!$rs->EOF):?>
        ...
        <td></td>
          ...
          <tr>
            <td width="60" align="center" valign="top" class="text_gray_normal"><input type="checkbox" name="chkbox[]" id="chkbox" value="<?php echo  $rs->fields['id'] ?>" onClick="swapCheckBox();" /></td>
              ...
              <td valign="top" class="text_gray_normal"><a href="image_product_edit.php?id=<?php echo $rs->fields['id'] ?>" class="text_gray_normal"><img src="../img_pro/thumbnail/<?php echo $rs->fields['image1']?>" border="0"></a></td>
