...
if($_GET[actions]=="del"){
	...
	$gettmp['fileid'] = $_GET['fid'];
	$gettmp['tid'] = $_GET['tid'];
	
		ADOdb_Active_Record::SetDatabaseAdapter($db);
			class tips_file extends ADOdb_Active_Record{}
			$tips_file = new tips_file();
			
					#	$gettemp[tid] = $_POST['tid'][$row];	
					$SQLstr = " SELECT * FROM tips_files WHERE tips_files.fileid =  '".$gettmp['fileid']."' ";
 					$rs = $db->Execute($SQLstr) ;
					
 					$fileatt = $rs->fields['fileatt'] ;
	  					if ($fileatt != "") :
							if(file_exists("../img_tips/file/$fileatt")) unlink("../img_tips/file/$fileatt");
	 					endif ;
					
					$tips_file->load("fileid=?", array($gettmp['fileid']));
					$tips_file->Delete();
	
				
		saverecord('Delete Press Center');	
		echo "<meta http-equiv=\"refresh\" content=\"2; URL=file_tips_to_delete3.php?tid=".$gettmp['tid']."\">";
		exit();
	#endif ;		
} // ลบ
?>
...
$SQLstr = "SELECT * FROM tips_files WHERE tid = '".$tid."' ORDER BY fileid DESC ";
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="100%" height="29"  border="0" cellpadding="0" cellspacing="0" background="images/bg_head05.gif">
          <tr>
            <td width="185" height="29"><span class="arialWH18B" style="margin-left:8px;">Delete File Center</span></td>
            <td align="right">&nbsp; &nbsp; </td>
            <td width="214">&nbsp; &nbsp;</td>
          </tr>
        </table></td>
      ...
      <tr>
        <td><form name="form1" method="post" action="file_tips_action.php" onSubmit="return checkForm();">
          ...
          <tr valign="top">
            ...
            <td><?php if (!$rs->EOF):?>
              <table width="100%" border="0" cellspacing="2" cellpadding="0" align="center">
                ...
                <tr>
                  <td width="30" align="center"><input type="checkbox" name="chkbox[<?php echo $rs->fields['fileid'] ?>]" id="chkbox" value="<?php echo $rs->fields['fileid'] ?>" onClick="swapCheckBox();"></td>
                  <td width="60" height="20" align="center" class="text_gray_normal"><a href="file_tips_to_edit4.php?fileid=<?php echo $rs->fields['fileid'] ?>&tid=<?php echo $rs->fields['tid'] ?>"class="text_gray_normal"><?php echo $rs->fields['fileid'] ?></a></td>
                  <td><span class="text_gray_normal"><a href="file_tips_to_edit4.php?fileid=<?php echo $rs->fields['fileid'] ?>&tid=<?php echo $rs->fields['tid'] ?>" class="text_violet_normal"><?php echo stripslashes($rs->fields['topic'])?></a></span></td>
                  <td width="80" align="center" class="arialGREY11nor"><?php if ($rs->fields['filestatus'] == 'Active'):
				echo '<b><font color="#0000FF">SHOW</font></b>';
				else:
				echo '<b><font color="#FF0000">HIDE</font></b>';
				endif; ?></td>
                  <td width="35" align="center"><a href="?actions=del&fid=<?php echo $rs->fields['fileid'] ?>&tid=<?php echo $rs->fields['tid']?>" onClick="return  confirm('Do you want to delete? ');"><img src="images/del_button.gif" width="13" height="13" border="0" /></a></td>
                </tr>
                <?php $rs->MoveNext(); ?>
                <?php endwhile; ?>
                <tr>
