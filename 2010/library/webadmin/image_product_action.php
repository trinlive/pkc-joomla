...
switch($_POST['MM_action']):
	...
	case 'create' : 
		#	$db->debug=1;
			$getdata[imagetopic]="Add Images Products";
				...
				ADOdb_Active_Record::SetDatabaseAdapter($db);
					class products_image extends ADOdb_Active_Record{}
						$products_image = new products_image();
						$products_image->tid = $gettmp['tid'];
						$products_image->caption = $gettmp['caption'];
						$products_image->image_active = $gettmp['image_active'];
				...
				if ($handle->uploaded) :
					...
					$handle->Process('../img_pro/fullsize/');
					if ($handle->processed) :
					$new_imagename = $handle->file_dst_name  ;
					$products_image->image2 = $new_imagename ;
					$handle->clean();
					else:
					echo 'error : ' . $handle->error;
					endif;
				###
				$thmhandle = new upload('../img_pro/fullsize/'.$handle->file_dst_name);
			...
			//	$thmhandle->image_ratio_x = true;
				$thmhandle->Process('../img_pro/thumbnail/');
	
					if ($thmhandle->processed):
					$new_imagename2 =  $thmhandle->file_dst_name ; 
					$products_image->image1 = $new_imagename2 ;
		#			$thmhandle->clean();
					else :
		...
		$imagecommit = $products_image->save();
		$id =  $tips_image->id ; //product id
	if($imagecommit) : // 
			$getdata[msg]="Add Images products Completed !!";
			$getdata[msg].="&nbsp;&nbsp;&nbsp;&nbsp;<span class='arialGray12'>|</span>&nbsp;&nbsp;&nbsp;&nbsp;<a href='image_product_new.php?tid=$gettmp[tid]'  class='arialVIO12B3'>Add Images product</a>";	
			saverecord('Add Images products');		
		else :
			$getdata[msg]="<span class=\"arialred12B\">Add Images products Not Completed !!</span><br>";
	...
	case 'update' :
		$getdata[imagetopic]="Edit Images products";		
		$gettmp[products_cate]= '1' ;
		...
		$gettmp[pimage2]=$_POST['pimage2'];      
			...
			class products_image extends ADOdb_Active_Record{}
				$products_image = new products_image();
				$products_image->load("id=?", array($gettmp[id]));
				$products_image->tid = $gettmp[tid];
				$products_image->caption = $gettmp['caption'];
				$products_image->image_active = $gettmp['image_active'];
				$products_image->replace();
		...
		if($products_image->replace()):// 	
			$getdata[msg]="Update Images products Completed !!";
			$getdata[msg].="&nbsp;&nbsp;&nbsp;&nbsp;<span class='arialGray12'>|</span>&nbsp;&nbsp;&nbsp;&nbsp;<a href='image_product_edit.php?id=$gettmp[id]&tid=$tid'  class='arialVIO12B3'>Edit Images products</a>";					
			saverecord('Edit Images products');				
		else://
	...
	case 'delete' : 
		$getdata[imagetopic]="Delete Image products";		
		$gettmp[products_cate]= '1' ;		
		$tid = $_POST['tid'];
			...
			$products_image = new products_image();
				...
				#	$gettemp[nid] = $_POST['nid'][$row];
					$SQLstr = "SELECT * FROM products_images WHERE id = '".$gettmpid."' ";
					...
					if ($gettmp[pimage1] != "") :
						if(file_exists("../img_pro/thumbnail/$gettmp[pimage1]")) unlink("../img_pro/thumbnail/$gettmp[pimage1]");
					endif ;
					if ($gettmp[pimage2] != "") :
						if(file_exists("../img_pro/fullsize/$gettmp[pimage2]")) unlink("../img_pro/fullsize/$gettmp[pimage2]");
					endif ; 
					
					$products_image->load("id=?", array($gettmpid));
					$products_image->Delete();
				endforeach ;
		
		if($products_image) : // 
			$getdata[msg]="Delete Images products Completed !!<br>";
			$getdata[msg].="<meta http-equiv=\"refresh\" content=\"5; URL=image_product_to_delete3.php?tid=$tid \">";	
			saverecord('Delete Images products');			
		else :
			$getdata[msg]="<span class=\"arialred12B\">Delete Images products Not Completed !!</span><br>";
			$getdata[msg].=$config[err][database];	
			$getdata[msg].="<br><a href='Javascript:history.back(1)'  class='arialBL12B'>Back</a>";
		endif ;			
	break;	
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="100%" height="29"  border="0" cellpadding="0" cellspacing="0" background="images/bg_head04.gif">
          <tr>
            <td width="276" height="29"><span class="arialWH18B" style="margin-left:8px;"><?php echo $getdata[imagetopic] ; ?></span></td>
