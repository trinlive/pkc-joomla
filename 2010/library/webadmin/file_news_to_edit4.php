...
$gettmp[nid] = $_GET[nid];
$SQLstr = "SELECT * FROM news_files WHERE news_files.nid = '".$gettmp[nid]."' AND news_files.fileid =  '".$gettmp[fileid]."'";
$rs = $db->Execute($SQLstr) ;
$stmt = "SELECT * FROM newses WHERE newses.nid = '".$gettmp[nid]."' ";
$rs2 = $db->Execute($stmt) ;
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" style="background:url(images/line_main.gif) repeat-y">
  <tr valign="top">
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td>
		<form action="file_action.php" method="POST" enctype="multipart/form-data" name="form1" style="margin:0" onSubmit="return checkForm();">
              ...
              <tr>
			   ...
			   <tr>
                  ...
                  <td valign="top"><?php if (!is_null($rs->fields['fileatt'])) {?>
                      <a href="../img_news/file/<?php echo $rs->fields['fileatt'];?>" target="_blank" class="text_gray_normal" title="<?php echo $rs->fields['fileatt'];?>"><?php echo $rs->fields['fileatt'];?></a>&nbsp;
