...
$SQLstr = " SELECT `newses`.`subject`,`news_images`.`id`,`newses`.`nid`,`news_images`.`image1`, `news_images`.`image_active` FROM `newses`Inner Join `news_images` ON `newses`.`nid` = `news_images`.`nid` AND `newses`.`nid` = `news_images`.`nid` where `newses`.`nid`= '$nid' ";
...
<html>
<head>
<title>:: CONTROL PANEL - SAKULTHIT CO., LTD. ::</title>
        ...
        <td></td>
                                ...
                                <tr>
                                  ...
                                  <td class="text_gray_normal"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    <tr>
                                      <td width="90"><a href="image_news_edit.php?id=<?php echo $rs->fields['id'] ?>&nid=<?php echo $rs->fields['nid'] ?>"><img src="../img_news/thumbnail/<?php echo $rs->fields['image1']?>" border="0"></a></td>
                                      <td>&nbsp;</td>
                                    </tr>
                                  </table></td>
