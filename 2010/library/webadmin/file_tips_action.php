...
switch($_POST[MM_action]){
	//  save 
	case "create" :{
		$getdata[imagetopic]="Add File Tips";
		...
		ADOdb_Active_Record::SetDatabaseAdapter($db);
			...
			$presscenter = new tips_file();		
			$presscenter->topic = $gettmp['topic'];
			$presscenter->filestatus = $gettmp['filestatus'];
			$presscenter->press_date = $gettmp['press_date'];
			$presscenter->tid = $gettmp['tid'];			
					
			if (is_uploaded_file($_FILES['fileatt']['tmp_name'])) :
				$foto_upload = new Foto_upload;
				$foto_upload->upload_dir = "../img_tips/file/"; // "files" is the folder for the uploaded files (you have to create these folder)
				$foto_upload->foto_folder ="../img_tips/file/"; 
	...
	case "update" :{
			...
			if (is_uploaded_file($_FILES['fileatt']['tmp_name'])) :
				...
				if ($foto_upload->upload()) :
					if ($presscenter->fileatt != "") :
						if(file_exists("../img_tips/file/$presscenter->fileatt")) unlink("../img_tips/file/$presscenter->fileatt");
					endif ;
					//	$foto_upload->process_image(false, false, true, 80); 
	 				$foto_upload->message[] = "Processed foto: ".$foto_upload->file_copy."!"; // "file_copy is the name of the foto"
		...
		if($presscenter->replace()) : 
			$getdata[msg]="Update File Completed !!";
		 	$getdata[msg].="&nbsp;&nbsp;&nbsp;&nbsp;<span class='arialGray12'>|</span>&nbsp;&nbsp;&nbsp;&nbsp;<a href='file_tips_to_edit4.php?fileid=".$gettmp[fileid]."&tid=".$gettmp[tid]."'  class='arialVIO12B3'>Edit File</a>";
	...
	case "delete" :{
		...
		ADOdb_Active_Record::SetDatabaseAdapter($db);
			...
			$presscenter = new tips_file();
				foreach($_POST[chkbox] AS $gettmp[fileid]) :
					...
					$SQLstr = " SELECT fileatt FROM `tips_files` WHERE `fileid` =  '".$gettmp[fileid]."' ";
		...
		if ($presscenter) :	
			...
			echo "<meta http-equiv=\"refresh\" content=\"2; URL=file_tips_to_delete.php\">";	
		else :
