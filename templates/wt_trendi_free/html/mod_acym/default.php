...
<table class="acym_form uk-table<?php if (!$displayInline) { echo " uk-table-small" ; } else { echo " uk-margin-remove-bottom"; } ?> uk-table-justify">
	<tr>
        <?php
        ...
        if (!empty($termslink) && !$displayInline) {
            echo '<td class="acyterms" '.($displayOutside && !$displayInline ? 'colspan="2"' : '').'>';
            echo '<input id="mailingdata_terms_'.$formName.'" class="uk-checkbox" type="checkbox" name="terms" title="'.acym_translation('ACYM_TERMS_CONDITIONS').'"/> '.$termslink;
            echo '</td>';
            if (!$displayInline) echo '</tr><tr>';
            }
        ?>
		<td <?php if ($displayOutside && !$displayInline) echo 'colspan="2"'; ?> class="acysubbuttons">
			...
			<input type="button" class="uk-button uk-button-primary" value="<?php echo acym_translation($subscribeText, true); ?>" name="Submit" onclick="try{ return submitAcymForm('subscribe','<?php echo $formName; ?>', 'acymSubmitSubForm'); }catch(err){alert('The form could not be submitted '+err);return false;}" />
            <?php if ($params->get('unsub', '0') == '1' && !empty($countUnsub)) { ?>
				<span style="display: none;"></span>
				<input type="button" class="uk-button uk-button-secondary" value="<?php echo acym_translation($unsubscribeText, true); ?>" name="Submit" onclick="try{ return submitAcymForm('unsubscribe','<?php echo $formName; ?>', 'acymSubmitSubForm'); }catch(err){alert('The form could not be submitted '+err);return false;}" />
            <?php } ?>
		</td>
	</tr>
</table>
<?php if (!empty($termslink) && $displayInline) {
        echo '<div class="acyterms" '.($displayOutside && !$displayInline ? 'colspan="2"' : '').'>';
        echo '<input id="mailingdata_terms_'.$formName.'" class="uk-checkbox" type="checkbox" name="terms" title="'.acym_translation('ACYM_TERMS_CONDITIONS').'"/> '.$termslink;
        echo '</div>';
    }
?>
